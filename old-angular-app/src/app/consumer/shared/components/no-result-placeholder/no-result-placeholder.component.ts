...
@Component({
  ...
  template: `<div class="no-result-placeholder-section">
                <div class="empty-result">
                    ...
                    <div *ngIf="config.buttons && config.buttons.length" class="action-buttons"><button class="btn btn-success btn-lg empty-result-action {{ button.type }}" *ngFor="let button of config.buttons" (click)="button.action()">{{ button.title }}</button></div>
                </div>
            </div>`,
  providers: []
})
