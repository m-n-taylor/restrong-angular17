...
export class ShoppingCart {
    ...
    constructor(private sharedData: SharedDataService, private constants: Constants) {
        if (Constants.DEBUG) {
            try { window['shoppingCart'] = this; } catch (e) { } // TODO: Remove this line in production
        }
    ...
    refresh = (ignoreSave?) => {
        ...
        for (var cartItemIndex in this.cartItems) {
            ...
            // Calculating `tax`
            cartItem.tax = Util.round(cartItem.subTotal * this.sharedData.data.taxPercent);

            // Calculating `delivery fees`
            if (this.sharedData.data.serviceType == this.constants.SERVICE_TYPE_DELIVERY) {
            ...
            cartItem.driverTip = Util.round(cartItem.subTotal * (this.sharedData.data.driverTipPercent / 100));

            // Calculating `total`
            cartItem.total = Util.round(cartItem.subTotal + cartItem.tax + cartItem.delivery + cartItem.driverTip);

            // Calculating `service fee`
            if (Util.isDefined(this.sharedData.data.serviceFee)) {
                cartItem.serviceFee = Util.round(cartItem.total * (this.sharedData.data.serviceFee.SecurityFee / 100) + (this.sharedData.data.serviceFee.TransactionFee));
            }
            else {
                cartItem.serviceFee = 0;
            }
