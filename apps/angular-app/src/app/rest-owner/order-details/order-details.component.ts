...
export class OrderDetailsComponent implements OnInit {
	...
	loadData = () => {
		...
		var globalSettingsPromise = this.ROService.getGlobalSettings().promise;

		Observable.forkJoin([restInfoPromise, orderDetailsPromise, globalSettingsPromise]).subscribe((response: any) => {
			...
			// Coupon Discount
			for (var i in this.order.OrderDetail.CouponDetail) {
				var couponDetail = this.order.OrderDetail.CouponDetail[i];

				if (couponDetail.DiscountPrice) {
					this.couponDiscount += couponDetail.DiscountPrice;
				}
			}
			...
			this.globalSettings = response[2];

			/**
			 * Deliver Text
			 */
			var deliverText = this.helperService.getDeliverText(this.order.OrderDetail.ServiceTypeID, this.helperService.allowDeliveryInSubscription(this.order.OrderDetail.Owner_SubscriptionID));
	...
	printOrder = () => {
		...
		if (this.order.OrderDetail.ServiceTypeID == this.constants.SERVICE_TYPE_ID_DELIVERY || this.order.OrderDetail.ServiceTypeID == this.constants.SERVICE_TYPE_ID_CATERING) {
			...
			if (!this.allowDeliveryInSubscription(this.order.OrderDetail.Owner_SubscriptionID)) {
				label += ' by Customer';
			}
		...
		// Dishzilla Service Charge
		calculationsBody.push([`Dishzilla Service Charge:`, this.currencyPipe.transform(this.order.OrderDetail.ServiceFee, 'USD', true) + ' -']);

		if (this.order.OrderDetail.ServiceTypeID == this.constants.SERVICE_TYPE_ID_DELIVERY || this.order.OrderDetail.ServiceTypeID == this.constants.SERVICE_TYPE_ID_CATERING) {

			if (!this.allowDeliveryInSubscription(this.order.OrderDetail.Owner_SubscriptionID)) {

				// Delivery Charge by Dishzilla
				calculationsBody.push([`Delivery Charge by Dishzilla:`, this.currencyPipe.transform(this.order.OrderDetail.Owner_DeliveryCharge_Restaurant, 'USD', true) + ' -']);
					...
					stack: [
						{
							...
							bold: true,
							alignment: 'center',
							text: `Subscription: ${this.order.OrderDetail.SubscriptionName}`
						},
						{
							fontSize: 15,
							bold: true,
						...
						body: [
							[
								[
									{
										margin: [0, 18, 70, 0],
										text: 'If you need to reach Dishzilla please feel free to call our restaurant customer service team at: ',
									},
									{
	...
	allowDeliveryInSubscription = (subscriptionID) => {
		return this.helperService.allowDeliveryInSubscription(subscriptionID);
	}

	goBack = () => {
		this.location.back();
		// this.router.navigate([`${Path.RO.BASE}/${Path.RO.MANAGE_RESTAURANT}`, this.fireFlyID], { queryParams: { tab: this.constants.TAB_ORDERS } });
