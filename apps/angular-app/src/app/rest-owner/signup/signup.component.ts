...
export class SignupComponent implements OnInit {
    ...
    public get pricingLink(): string {
        var link = '/backoffice/pricing/';

        if (Util.isDefined(this.selectedPlan)) {
            var plan = this.helperService.getSelectedPlan(this.pricingList, this.selectedPlan);

            if (plan) {
                link += this.constants.PRICING_TITLE[plan.Type];
            }
        }
    ...
    constructor(@Inject(PLATFORM_ID) private platformId: Object, public constants: Constants, private ROService: ROService, private userService: UserService, private router: Router, public input: InputService, public sharedDataService: SharedDataService, private route: ActivatedRoute, private toastr: ToastsManager, private helperService: ROHelperService) {
        ...
        this.ROService.getGlobalSettings().promise.subscribe((response: any) => {
            ...
            for (var i in pricing) {
                ...
                if (item.ID == this.constants.PRICING_MARKETPLACE) {
                    ...
                    if (Util.isDefined(this.pricingMarketPlace) && Util.isDefined(this.pricingMarketPlace.Details) && this.pricingMarketPlace.Details.length > 0) {
                        this.selectedPlan = this.pricingMarketPlace.Details[0].ID;
                    }
                }
            ...
            if (Util.isDefined(this.pricingMarketPlace)) {
                this.pricingList.push(this.pricingMarketPlace);
            }

            if (Util.isDefined(this.pricingPlatform)) {
                this.pricingList.push(this.pricingPlatform);
            }

            if (Util.isDefined(this.pricingCombo)) {
                this.pricingList.push(this.pricingCombo);
            }

            // this.pricingList = [this.pricingMarketPlace, this.pricingPlatform, this.pricingCombo];

            this.route.queryParams.subscribe((params: any) => {
                if (Util.isDefined(params.plan)) {
    ...
    continueStepEmail = () => {
        ...
        requestData.plan = this.selectedPlan;
    ...
    registerNewRest = () => {
        ...
        requestData.t = this.newRest.Title;
