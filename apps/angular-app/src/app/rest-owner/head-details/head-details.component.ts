...
export class HeadDetailsComponent implements OnInit {
	...
	viewMenuItemDetails = (menuItem?: ROMenuItem) => {
	...
	toggleShowImageInGalleryMenuItem = (menuItem: ROMenuItem) => {
		Util.log(this.LOG_TAG, 'toggleShowImageInGalleryMenuItem', menuItem);

		this._saveMenuItem(menuItem);
	}

	toggleActiveMenuItem = (menuItem: ROMenuItem) => {
		Util.log(this.LOG_TAG, 'toggleActiveMenuItem', menuItem);

		this._saveMenuItem(menuItem);
	}

	private _saveMenuItem = (menuItem: ROMenuItem) => {
		...
		this.ROService.updateMenuItem(requestData).subscribe((response: any) => {
			...
			if (response.Status == this.constants.STATUS_SUCCESS) {
				...
				if (response.Code == 'ERR_SHOWIMAGEINGALLERY') {
					menuItem.ShowImageInGallery = false;
					this.toastr.error('Menu item must have an image before you can enable Show In Gallery.', 'Error!');
				}
				else {
					this.toastr.error('Unable to update item, Please try later.', 'Error!');
				}
	...
	deleteMenuItem = (menuItemIndex, menuItem: ROMenuItem) => {
