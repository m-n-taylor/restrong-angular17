...
export class MenuItemDetailsComponent implements OnInit {
	...
	loadData = () => {
		...
		if (this.menuItemID) {
			promiseList.push(this.loadMenuItemSizes());
		...
		Observable.forkJoin(promiseList).subscribe(response => {
			...
			var masterHeadInfoResponse: any = response[i++];
			this.masterHead = masterHeadInfoResponse.Data;

			var headInfoResponse: any = response[i++];
			this.head = headInfoResponse.Data;

			if (this.menuItemID) {

				var menuItemSizesResponse: any = response[i++];
				this.menuItemSizes = menuItemSizesResponse.Data;

				var optionGroupSuggestionResponse: any = response[i++];
				this.optionGroupSuggestionList = optionGroupSuggestionResponse.Data;

				var menuItemInfoResponse: any = response[i++];
				this.menuItem = menuItemInfoResponse.Data;

				var menuItemSizeListResponse: any = response[i++];
				this.menuItemSizeList = menuItemSizeListResponse.Data;

				var menuOptionListResponse: any = response[i++];
	 ...
	 * Menu Item Size
	 */
	toggleMenuItemSizesSection = () => {
		this.menuItem.Is_Single_Size = false;

		this.isOpenMenuItemSizesSection = !this.isOpenMenuItemSizesSection;
	}
	...
	loadMenuItemSizes = () => {
		...
		return this.ROService.getMenuItemSizes(requestData);
	}
	...
	saveMenuItemSizeSortOrder = () => {
		...
		for (var index in this.menuItemSizeList) {
			...
			if (Util.isDefined(menuItemSize.ID)) {
	...
	addMenuItemSize = () => {
		if (this.isOpenMenuItemSizesSection) {
			this.toggleMenuItemSizesSection();
		}
	...
	saveSelectedMenuItemSize = () => {
		var selectedMenuItemSizeList = this.menuItemSizes.filter(s => s.isSelected);

		for (var i in selectedMenuItemSizeList) {
			var item = selectedMenuItemSizeList[i];

			var menuItemSize = this.addMenuItemSize();
			menuItemSize.Name = item.Name;
			// menuItemSize.Price = 0;
			// this._saveMenuItemSize(menuItemSize, null);
		}
	}
	...
	toggleOptionGroupSuggestionSection = () => {
		this.isOpenOptionGroupSuggestionSection = !this.isOpenOptionGroupSuggestionSection;
	}

	toggleOptionGroupSuggestion = () => {
		this.isOpenOptionGroupSuggestion = !this.isOpenOptionGroupSuggestion;

		if (!this.isOpenOptionGroupSuggestion) {
			this._deactiveOptionGroupSuggestionItemDetails(this.selectedOptionGroupSuggestionItem);
	...
	chooseOptionGroupSuggestionItem = (selectedItem) => {
		this._deactiveOptionGroupSuggestionItemDetails(selectedItem);
	...
	private _deactiveOptionGroupSuggestionItemDetails = (selectedOptionGroupSuggestionItem) => {
		if (Util.isDefined(selectedOptionGroupSuggestionItem)) {
			for (var i in selectedOptionGroupSuggestionItem.Details) {
				var item = selectedOptionGroupSuggestionItem.Details[i];
				item.isActive = false;
			}
		}
	}
	...
	saveSelectedOptionGroupSuggestion = () => {
		if (!this.selectedOptionGroupSuggestionItem) {
			this.toastr.error(`Please choose an option category first.`, 'Error!');
		}
		else {
			this.busy = true;
			...
			this._saveMenuOption(menuOption).subscribe(response => {
				if (response.Status == this.constants.STATUS_SUCCESS) {
					...
					if (promiseList.length > 0) {
						Observable.forkJoin(promiseList).subscribe((response: any) => {
							...
							this.toastr.success(`${menuOption.Name} added successfully.`, 'Success!');
							this.busy = false;
						});
					}
					else {
					...
					if (response.Code == 'ERR_NAME_EXIST') {
						this.toastr.error(`${menuOption.Name} already exists, Please try a different name.`, 'Sorry!');
					}
					else {
				...
				this.toggleOptionGroupSuggestion();
			});
		}
	}
	...
	saveMenuOptionSortOrder = () => {
		...
		for (var index in this.menuOptionList) {
			...
			if (Util.isDefined(menuOption.ID)) {
				body.SortDetails.push({
					ID: menuOption.ID,
	...
	openMenuOption = (menuOption: MenuOption) => {
		...
		this.menuOptionAddLimitation = this.menuOption.Minimum_Select != 0 || this.menuOption.Maximum_Select != 0;
	...
	menuOptionAddLimitationChanged = () => {
		if (!this.menuOptionAddLimitation) {
			this.menuOption.Minimum_Select = 0;
			this.menuOption.Maximum_Select = 0;
		}
	}
	...
	saveMenuOptionItemSortOrder = () => {
		...
		for (var index in this.menuOptionItemList) {
			...
			if (Util.isDefined(menuOptionItem.ID)) {
				body.SortDetails.push({
	...
	setDefaultMenuOptionItem = (menuOptionItem: MenuOptionItem) => {
		for (var index in this.menuOptionItemList) {
			var optionItem = this.menuOptionItemList[index];

			if (!optionItem.editMode && optionItem.Is_Default) {
				this.oldDefaultMenuOptionItem = optionItem;
			}

			optionItem.Is_Default = false;
		}

		menuOptionItem.Is_Default = true;

		if (!menuOptionItem.editMode) {
