...
// Polyfill `intl` import for currency BUG (internalization API) in Safari
import 'intl';
...
export class EditRestaurantComponent implements OnInit {
    TAB_REST_INFO = this.constants.TAB_REST_INFO;
    TAB_MENUS = this.constants.TAB_MENUS;
    TAB_ORDERS = this.constants.TAB_ORDERS;
    TAB_COUPONS = this.constants.TAB_COUPONS;
    TAB_PREVIEW = this.constants.TAB_PREVIEW;
    TAB_CHARTS = this.constants.TAB_CHARTS;
    TAB_FIN_REPORT = this.constants.TAB_FIN_REPORT;
    TAB_FIN_STATEMENT = this.constants.TAB_FIN_STATEMENT;
    TAB_CUSTOMER_REVIEW = this.constants.TAB_CUSTOMER_REVIEW;
    ...
    restInfoTab = {
        ...
        birthdayDTOptions: <DateTimeOptions>{
            hideClear: true
        },
        subscription: <any>{},
        DeliveryHours: <any>[],
    ...
    subscriptionRequestSelectRestTab = null;
    ...
    constructor( @Inject(PLATFORM_ID) private platformId: Object, public constants: Constants, private ROService: ROService, private router: Router, private route: ActivatedRoute, public input: InputService, public sharedDataService: SharedDataService, public helperService: ROHelperService, private breadcrumbService: BreadcrumbService, private toastr: ToastsManager, private eventsService: EventsService, private datePipe: DatePipe, private currencyPipe: CurrencyPipe, private decimalPipe: DecimalPipe, private app: ApplicationRef, private componentFactoryResolver: ComponentFactoryResolver, private vcr: ViewContainerRef, private zone: NgZone, public userService: UserService) {
        ...
        this.subscriptionRequestSelectRestTab = this.eventsService.requestSelectRestTab.subscribe((data) => {
            Util.log(this.LOG_TAG, 'requestSelectRestTab', data);
        ...
        if (isPlatformBrowser(this.platformId)) {
            ...
            Observable.combineLatest([this.route.queryParams, this.route.params]).subscribe(data => {
                ...
                if (this.fireFlyID != params.fireFlyID) {
                    ...
                    promises.push(this.ROService.getGlobalSettings().promise);

                    promises.push(this.ROService.getAllStates());

                    var requestData = new ROAPIRequestData();
                    ROAPIRequestData.fillFireFlyID(requestData, this.fireFlyID);

                    promises.push(this.ROService.getRestInfo(requestData));

                    Observable.forkJoin(promises).subscribe((response: any) => {
                        ...
                        var globalSettings: any = response[index++];

                        var pricing = globalSettings.Pricing;

                        var pricingMarketPlace: any = {};
                        var pricingPlatform: any = {};
                        var pricingCombo: any = {};

                        for (var i in pricing) {
                            var item = pricing[i];

                            if (item.ID == this.constants.PRICING_MARKETPLACE) {
                                pricingMarketPlace = item;
                            }

                            if (item.ID == this.constants.PRICING_PLATFORM) {
                                pricingPlatform = item;
                            }
                        ...
                        this.pricingList = [pricingMarketPlace, pricingPlatform, pricingCombo];

                        /**
                         * All States
                         */
                        var stateList = response[index++];

                        this.stateList = Util.clone(stateList);
                        this.CIStateList = Util.clone(stateList);

                        /**
                         * Rest Info
                         */
                        var restInfoResponse = response[index++];
                        this.restInfoTab.rest = restInfoResponse.Data;

                        // State
                        if (Util.isDefined(this.restInfoTab.rest.State)) {
                            var tempState = this.stateList.filter(s => s.State == this.restInfoTab.rest.State);

                            if (tempState.length > 0) {
                                tempState[0].isSelected = true;
                            }
                        }

                        // CI State
                        if (Util.isDefined(this.restInfoTab.rest.CI_State)) {
                            var tempCIState = this.CIStateList.filter(s => s.State == this.restInfoTab.rest.CI_State);

                            if (tempCIState.length > 0) {
                                tempCIState[0].isSelected = true;
                            }
                        }

                        this.restInfoTab.rest.ConfirmAccountNumber = this.restInfoTab.rest.AccountNumber;
                        this.restInfoTab.restCopy = Util.clone(this.restInfoTab.rest);

                        this.restInfoTab.WorkingHours = restInfoResponse.WorkingHours;
                        ...
                        this.restInfoTab.DeliveryHours = restInfoResponse.DeliveryHours;
                        ...
                        this.restInfoTab.Holidays = restInfoResponse.Holidays || [];
                        this.restInfoTab.HolidaysCopy = Util.clone(this.restInfoTab.Holidays);

                        this.restInfoTab.deliveryZoneList = restInfoResponse.DeliveryZone || [];
                        ...
                        Util.log(this.LOG_TAG, 'getRestInfo', restInfoResponse);
                    });
                }
                else {
    ...
    private _selectTab = (tab) => {
        ...
        this.eventsService.onRestTabSelected.emit({ tab: this.activeTab });

        if (this.activeTab !== this.TAB_CHARTS) {
            sessionStorage.removeItem('otherRestsFF');
        }
    }
    ...
    onSubscriptionChange = () => {
        ...
        for (var i in this.pricingList) {
            var pricing = this.pricingList[i];

            for (var j in pricing.Details) {
                var detail = pricing.Details[j];

                if (detail.ID == subID) {
                    this.restInfoTab.subscription = detail;

                    if (detail.ServiceFee) {
                        this.restInfoTab.subscription.ServiceFeePercent = Util.round(detail.ServiceFee * 100);
                    }

                    this.restInfoTab.subscription.type = pricing.name;

                    this.pricingLink = '/backoffice/pricing/' + this.constants.PRICING_TITLE[this.restInfoTab.subscription.Type];
    ...
    saveRestInfo = (event, form) => {
        ...
        // State
        var selectedStates = this.stateList.filter(s => s.isSelected);
        if (selectedStates.length > 0) {
            this.restInfoTab.stateError = null;
            this.restInfoTab.rest.State = selectedStates[0].State;
        }
        else {
            this.restInfoTab.stateError = 'State is required.';
            validInputs = false;
        }

        // CI State
        var selectedCIStates = this.CIStateList.filter(s => s.isSelected);
        if (selectedCIStates.length > 0) {
            this.restInfoTab.CIStateError = null;
            this.restInfoTab.rest.CI_State = selectedCIStates[0].State;
        }
        else {
            this.restInfoTab.CIStateError = 'State is required.';
            validInputs = false;
        }
        ...
        if (form.valid && validInputs && validWorkingHours && validDeliveryHours && validHolidays) {
    ...
    private _saveRestInfo = (enableRestaurant, acceptTerms?: boolean) => {
        ...
        Observable.forkJoin(promiseList).subscribe((response: any) => {
            ...
            var restInfoResponse = response[0];
            ...
            if (restInfoResponse.Status == this.constants.STATUS_ERROR) {
                this.toastr.error('Unable to save, Form contains some errors.', 'Sorry!');
            }
            else {
    ...
    loadMenusTab = () => {
        ...
        this.loadMenusDataStatus();

        this.menusTab.tabDataInterval = setInterval(() => {
            this.menusTab.busyText = 'Please wait, We are preparing your menu...';
            this.loadMenusDataStatus();
        }, 3000);
    }

    loadMenusDataStatus = () => {
        Util.log(this.LOG_TAG, 'loadMenusTabStatus()');
        ...
        this.ROService.getRestStatus(requestData).subscribe(response => {
            if (response.Status == this.constants.STATUS_SUCCESS) {
                var data = response.Data;

                if (data.EnableOnlineOrdering) {
                    if (this.menusTab.tabDataInterval) {
                        clearInterval(this.menusTab.tabDataInterval);
                        this.menusTab.tabDataInterval = null;
                    }

                    this.menusTab.busyText = null;
                    this.loadMenusTabData();
                }
            }
    ...
    loadMenusTabData = () => {
        Util.log(this.LOG_TAG, 'loadMenusTabData()');
    ...
    saveMasterHeadsSortOrder = () => {
        ...
        for (var index in this.menusTab.masterHeads) {
            ...
            if (Util.isDefined(masterHead.ID)) {
                body.SortDetails.push({
                    ID: masterHead.ID,
    ...
    ordersSearchTextEnter = (clearText) => {
        Util.log(this.LOG_TAG, 'ordersSearchTextEnter()');

        if (clearText) {
            this.ordersTab.searchText = '';
        }
    ...
    ordersTabPageChange = () => {
        ...
        Util.log(this.LOG_TAG, 'ordersTabDataPageChange', queryParams);
    }
    ...
    loadFinReportTab = () => {
        ...
        this.ROService.getFinancialReport(requestData).subscribe(response => {
            ...
            Util.log(this.LOG_TAG, 'getFinancialReport', response);
        });
    }
    ...
    printFinStatement = () => {
        ...
        var ordersTableBody: any = [[
            { text: 'Order Date/Time', style: 'ordersTableHeader' },
            { text: 'Order Number', style: 'ordersTableHeader' },
            { text: 'Order Status', style: 'ordersTableHeader' },
            ...
            { text: 'Coupon Discount', style: 'ordersTableHeader' },
            ...
            { text: 'Dishzilla Service Charge', style: 'ordersTableHeader' },
        ...
        for (var index in this.finStatementTab.Details) {
            ...
            ordersTableBody.push([
                ...
                { text: item.OrderStatus, style: 'ordersTableRow', bold: true },
                ...
                { text: item.Discount ? this.currencyPipe.transform(item.Discount, 'USD', true) : '-', style: 'ordersTableRow' },
                { text: this.currencyPipe.transform(item.DeliveryCharge || 0, 'USD', true), style: 'ordersTableRow' },
                { text: this.currencyPipe.transform(item.Tax_Final || 0, 'USD', true), style: 'ordersTableRow' },
                { text: this.currencyPipe.transform(item.Driver_Tip || 0, 'USD', true), style: 'ordersTableRow' },
                { text: this.currencyPipe.transform(item.GrandTotal || 0, 'USD', true), style: 'ordersTableRow' },
                { text: this.currencyPipe.transform(item.Owner_ServiceFee || 0, 'USD', true), style: 'ordersTableRow' },
                { text: this.currencyPipe.transform(item.Owner_NetAccountCredit || 0, 'USD', true), style: 'ordersTableRow' },
            ]);
        }
            ...
            content: [
                {
                    ...
                    columns: [
                        {
                            image: this.constants.BRAND_LOGO_BASE64,
                            width: 176,
                            height: 24,
                        },
                        {
    ...
    ngOnDestroy() {
        ...
        if (this.subscriptionRequestSelectRestTab) {
            this.subscriptionRequestSelectRestTab.unsubscribe();
        }

        this.eventsService.onRestTabSelected.emit({ tab: null });
    }
}