 ...
 */
@Injectable()
export class ROHelperService {
    LOG_TAG = 'ROHelperService =>';

    constructor(public constants: Constants, public sharedDataService: SharedDataService, private ROService: ROService) {
    ...
    allowDeliveryInSubscription = (subscriptionID) => {
        ...
        for (var i in this.sharedDataService.globalSettings.Pricing) {
            var pricing = this.sharedDataService.globalSettings.Pricing[i];

            var temp = pricing.Details.filter(p => p.ID == subscriptionID);
    ...
    getDeliverText = (serviceTypeID, allowDelivery) => {
        var deliverText = '';
        ...
        // Delivery OR Catering
            ...
            if (allowDelivery) {
                ...
                deliverText = 'Pickup by Dishzilla.com at';
            }
    ...
    loadMenuItemSizes = (fireFlyID: string, menuItem: ROMenuItem, menuItemSizeID: number) => {
        Util.log(this.LOG_TAG, 'loadMenuItemData()');
        ...
        return new Promise<any>((resolve, reject) => {
            ...
            if (!menuItem.Is_Single_Size) {
                ...
                requestData.mid = menuItem.ID;

                this.ROService.getMenuOptionSizePreview(requestData)
                    .subscribe(response => {
                        menuItem.menuItemSizes = response.Data;

                        var isSizeFound = false;

                        for (var i in menuItem.menuItemSizes) {
                            var item = menuItem.menuItemSizes[i];

                            if (menuItemSizeID) {
                                if (item.id == menuItemSizeID) {
                                    isSizeFound = true;
                                }
                            }
                            ...
                            if (isSizeFound) {
                                this.loadMenuItemOptionItems(fireFlyID, menuItem, item).then(res => {
                                    resolve();
                                }).catch(err => {
                                    reject();
                                });
                ...
                this.loadMenuItemOptionItems(fireFlyID, menuItem, null)
                    .then(res => {
                        resolve();
                    }).catch(err => {
                        reject();
    ...
    loadMenuItemOptionItems = (fireFlyID: string, menuItem: ROMenuItem, menuItemSize) => {
        menuItem.busy = true;

        return new Promise<any>((resolve, reject) => {
            menuItem.selectedMenuItemSize = menuItemSize;
            ...
            this.ROService.getMenuOptionsPreview(requestData)
                .subscribe(response => {
                    menuItem.menuItemOptions = response.MenuOptions;
                    menuItem.busy = false;
                    resolve();

                    Util.log(this.LOG_TAG, 'getMenuOptionsPreview', response);
                }, err => {
                    menuItem.busy = false;
                    reject();
                });
    ...
    getSelectedPlan = (pricingList: Array<any>, selectedPlan: number) => {
        var foundPlan = null;

        for (var i in pricingList) {
            var pricing = pricingList[i];

            for (var j in pricing.Details) {
                var detail = pricing.Details[j];

                if (detail.ID == selectedPlan) {
                    foundPlan = detail;
