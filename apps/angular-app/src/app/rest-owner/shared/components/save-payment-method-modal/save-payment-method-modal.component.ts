...
declare var document, google, moment;

@Component({
	selector: 'save-payment-method-modal',
	templateUrl: './save-payment-method-modal.component.html',
	
})
export class SavePaymentMethodModalComponent extends BaseModal {

	/**
	 * Properties
	 */
	readonly LOG_TAG = 'SavePaymentMethodModalComponent';
	...
	initVariables = () => {
		...
		// TODO: needs to get these `months` and `years` from `Constants`
			...
			if (newFFIDs.length == 0) {
				...
				this.ROService.saveUser({ user: this.user }).subscribe((userResponse: any) => {
					...
					if (userResponse.Status == this.constants.STATUS_SUCCESS) {
						...
						this.toastr.error('Sorry, Unable to save item');
