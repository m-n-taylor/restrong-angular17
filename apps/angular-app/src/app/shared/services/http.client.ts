...
export class HttpClient {
    ...
    static get parameters() {
        return [HttpTransferService, Http, Constants, EventsService]; //ModelService, 
    }

    constructor(private tHttp: HttpTransferService, private http: Http, private constants: Constants, private eventsService: EventsService) {
    ...
    private _responseInterceptor = (res) => {
        ...
        if (Util.isDefined(response.Status) && response.Status == this.constants.STATUS_ERROR && Util.isDefined(response.Code) && response.Code == 'ERROR_INVALID_AUTHCODE') {
            Util.log(this.LOG_TAG, 'CR Auth Expired');
