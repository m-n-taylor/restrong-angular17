...
@Component({
    ...
    template: `<div *ngIf="input.errors && (input.dirty && input.touched || form.submitted)">
                ...
                <div *ngIf="input.errors?.digits" class="help-block f-bold">{{ label }} must be a digit</div>
                ...
                <div *ngIf="input.errors.mValidPhone" class="help-block f-bold">{{ label }} must be 10 digits only.</div>
                <div *ngIf="input.errors.mValidateEqual" class="help-block f-bold">{{ label }} must be a equal to {{ labelEqual }}</div>
                <div *ngIf="input.errors.mValidateNumber" class="help-block f-bold">
                    {{ label }} {{ input.errors.mValidateNumber.mValidateNumberMsg ? input.errors.mValidateNumber.mValidateNumberMsg :' is not valid' }}
                </div>
                <div *ngIf="input.errors.mValidState" class="help-block f-bold">
                    {{ label }} {{ input.errors.mValidState.mValidStateMsg ? input.errors.mValidState.mValidStateMsg :' is not valid' }}
                </div>
